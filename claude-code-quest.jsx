...
const ClaudeCompanion = ({ size=2.5 }) => {
  ...
  const c = {P:"#D4A574",W:"#FFFFFF",M:"#E88B8B",C:"#7B68EE"};
  return (
...
const getLevel=xp=>{let l=1;for(let i=1;i<LVL.length;i++){if(xp>=LVL[i])l=i+1;else break;}return l;};
const getXpNext=lv=>LVL[lv]||LVL[LVL.length-1];
const getXpCur=lv=>LVL[lv-1]||0;
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
const NAMES={narrator:"ナレーション",cto:"CTO 田中",companion:"クロード"};
...
export default function ClaudeCodeQuest() {
  ...
  useEffect(()=>{(async()=>{try{const r=await window.storage.get("ccq_save");if(r?.value){const d=JSON.parse(r.value);if(d.xp)setXp(d.xp);if(d.cl)setCleared(d.cl);}}catch{}})();},[]);
  const saveProgress=async(nxp,ncl)=>{const c=ncl||cleared;try{await window.storage.set("ccq_save",JSON.stringify({xp:nxp,cl:c}));}catch{}};
  ...
  const addTL=lines=>{setTermHistory(h=>[...h,...lines]);scrollTerm();};

  const playAttack=async dmg=>{
  ...
  const handleAiResponse=async query=>{
    ...
    try{const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},
      ...
      const data=await res.json();const text=data.content?.map(c=>c.text||"").join("")||"応答を取得できませんでした";
  ...
  const handleCommand=async()=>{
    ...
    if(isMatch){
      ...
      else if(step.response[0]?.type==="install"){setInstalling(true);
  ...
  const analyzeFailure=async(cmd,cs)=>{
    setAiLoading(true);
...
【ルール】答えは直接教えない/入力と正解の具体的な違いを分析/間違い回数が多いほど具体的に/RPGの仲間らしい口調で2-3文/CLI初心者向け`,
      ...
      const data=await res.json();setCompanionMsg(data.content?.map(c=>c.text||"").join("")||"もう一度確認してみよう！");
...
【ルール】${fc===0?"方向性だけ教えて":fc===1?"軽いヒント":fc===2?"もう少し具体的に":fc>=3?"かなり踏み込んだヒント":""}/答えそのものは完全には教えない/RPGの仲間らしい口調で2-3文`,
      ...
      const data=await res.json();setCompanionMsg(data.content?.map(c=>c.text||"").join("")||step.hint);
  ...
  const startStage=idx=>{if(!isUnlocked(idx))return;
        ...
        <div style={S.storyWrap}>
          ...
          <div style={S.dlgWrap} onClick={advanceDialog}>
            <div style={S.dlgBox}>
              ...
              <p style={{...S.dlgText,...(isN?{fontStyle:"italic",textAlign:"center",color:"#8A7A5A"}:{})}}>{displayedText}</p>
